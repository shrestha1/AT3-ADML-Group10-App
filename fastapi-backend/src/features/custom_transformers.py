#Full defining of all Pipeline Functions 
##Pipeline Customer Transfomers 
##Used for Alistair Schillert's model
...
def run_pipeline(origin_airport, destination_airport, cabin_type, flight_date, flight_time):
    # Ensure flight_time includes seconds
    if isinstance(flight_time, str):
        # Split the flight_time by ':' to extract hours and minutes
        time_parts = flight_time.split(':')
        
        if len(time_parts) == 2:
            # If only HH:MM is provided, append ':00' for seconds
    ...
    df = pd.DataFrame({
        ...
        'cacabin_type': [cabin_type],
        'flight_date': [flight_date],        
        'flight_time': [flight_time]  # Ensure flight_time is in HH:MM:SS format
    })
    coordinates_df = pd.DataFrame({
    'IATA': ['ORD', 'BOS', 'LAX', 'LGA', 'ATL', 'DFW', 'JFK', 'EWR', 'MIA', 'CLT', 'SFO', 'DEN', 'DTW', 'PHL', 'IAD', 'OAK'],
    'latitude': [41.9786, 42.3656, 33.9416, 40.7769, 33.6407, 32.8998, 40.6413, 40.6895, 25.7959, 35.2140, 37.6213, 39.8561, 42.2124, 39.8744, 38.9531, 37.7126],
    ...
    with open('./models/alistair/pipeline_airport_alistair_externalv3.pkl', 'rb') as file:
